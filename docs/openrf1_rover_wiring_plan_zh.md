...
- 电池圆孔插头极性、BMS 连续/峰值电流、主保险丝、主回路线径、板端
  电源插头规格和三路 HC-SR04 ECHO 高电平尚未实测。
- 硬件组已报告三路 HC-SR04 避障可运行；这不是当前仓库安全固件、
  电气输入兼容性、ESP32 通信或整车联合功能已经验收的证据。
...
| 9 | 10 kOhm，建议 1% | 1 个 Hall 分压上臂 + 8 个编码器上拉 |
| 1 | 15 kOhm，建议 1% | 1 个 Hall 分压下臂 |
| 3 组备用 | 分压电阻或合适的电平转换器 | 三路 ECHO 实测超限时整改 |
...
当前实车的三路 ECHO 均未安装分压，用户报告避障功能可正常工作；这只证明
当前组合能够运行，不等于已经验证 STM32 输入电气安全。以下分压图是测得
ECHO 高电平超出所选输入路径限制时的整改方案，不代表当前接线：
...
| 编号 | 位置 | TRIG | ECHO | 当前状态 |
| --- | --- | --- | --- | --- |
| ultrasonic_1 | 左 | H3-1 PB9 | H3-2 PB8 | 用户确认的当前演示映射 |
| ultrasonic_2 | 中 | H3-3 PB5 | H3-4 PB4 | 用户确认的当前演示映射 |
| ultrasonic_3 | 右 | H3-5 PD2 | H3-6 PC11 | 用户确认的当前演示映射 |

运行时只能错峰触发，不能三个同时发射。用户报告当前三路 ECHO 都没有
分压并且避障可以工作；这只能记为“功能运行现象”，不能证明电气安全。
PB8、PB4、PC11 上的 ECHO 高电平仍需实测，测量前不得声明直连已通过
电气验收。历史 CN6 PA5/PA4 隔离测试方案继续保留，但不再是当前演示映射。
...
纠正后的 Hall 感应点距离为：到车头边界 185 mm、车尾边界 135 mm、
左右边界各 75 mm、前后轮轴中心各 95 mm。结合 190 mm 轴距，可确认
Hall 相对四轮轴心矩形中心的平面偏移为 `base_link x=0 mm、y=0 mm`；
相对 320 x 150 mm 车体外形中心则为 x=-25 mm、y=0 mm（+x 向前，
+y 向左）。用户提供的 Hall 感应点离地高度为 65 mm；结合 39.5 mm
负载轮半径，可推导 `base_link z=+25.5 mm`。感应面方向和可靠磁触发
距离仍未验证。
...
- [ ] Hall 的 10 kOhm/15 kOhm 分压器阻值和中点已核对。
- [ ] PB8、PB4、PC11 上的 ECHO 高电平已实测并与准确的板级输入限制核对；如超限，已增加独立分压或电平转换。
- [ ] H5-1、H6-1 均未接线。
- [ ] ESP32 外部 5 V 跳线可拔除。
