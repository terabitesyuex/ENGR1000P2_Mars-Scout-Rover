...
static uint16_t connector_scale_permille(uint8_t connector) {
    static const uint16_t scales[4] = {
        OPENRF1_DEMO_CN1_SPEED_SCALE_PERMILLE,
        OPENRF1_DEMO_CN2_SPEED_SCALE_PERMILLE,
        OPENRF1_DEMO_CN3_SPEED_SCALE_PERMILLE,
        OPENRF1_DEMO_CN4_SPEED_SCALE_PERMILLE
    };
    return connector >= 1u && connector <= 4u ? scales[connector - 1u] : 0u;
}
...
static void motor_set_raw(uint8_t connector, uint16_t duty_permille, int8_t direction) {
    uint32_t compare =
        ((uint32_t)(OPENRF1_DEMO_PWM_AUTO_RELOAD + 1u) * duty_permille) / 1000u;
    ...
    if (direction >= 0) {
        direction_port->BRR = (uint32_t)1u << direction_pin;
        *capture_compare = (uint16_t)compare;
    } else {
        direction_port->BSRR = (uint32_t)1u << direction_pin;
        *capture_compare = (uint16_t)reverse_compare;
    }
}
...
static void motor_set_physical_connector(
    ...
    duty_permille = electrical_direction >= 0 ?
        OPENRF1_DEMO_ELECTRICAL_FORWARD_DUTY_PERMILLE :
        OPENRF1_DEMO_ELECTRICAL_REVERSE_DUTY_PERMILLE;
    duty_permille = (duty_permille * connector_scale_permille(connector)) / 1000u;
    if (duty_permille > 1000u) {
        duty_permille = 1000u;
    }
    motor_set_raw(connector, (uint16_t)duty_permille, electrical_direction);
}

void demo_platform_set_motion(DemoMotion motion) {
    int8_t directions[4] = {0, 0, 0, 0};
    uint8_t connector;

    demo_platform_stop_all();
    if (motion == DEMO_MOTION_FORWARD) {
        directions[0] = -1; directions[1] = -1;
        directions[2] = -1; directions[3] = -1;
    ...
    for (connector = 1u; connector <= 4u; ++connector) {
        motor_set_physical_connector(connector, directions[connector - 1u]);
    }
}
...
static void ultrasonic_gpio_init(void) {
    ...
    /* Release PB4 from JTAG while retaining SWD access. */
    ...
    for (index = 0u; index < 3u; ++index) {
        ...
        gpio_configure(g_echo_pins[index].port, g_echo_pins[index].pin, 0x04u);
    }
}
