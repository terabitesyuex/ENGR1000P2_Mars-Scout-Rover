...
Recorded manual evidence in `evidence/phase3.2d/` marks isolated MPU6050 firmware flashing/execution, H4 wiring and idle voltage checks, I2C ACK at `0x68`, WHO_AM_I `0x68`, isolated configuration readback, live IMU JSON telemetry, startup gyro-bias calibration, `gyro_raw` versus bias-corrected `gyro_dps` semantics, approximately 10 Hz telemetry, a 15-second no-sequence-loss capture, and isolated axis response as MANUAL_EVIDENCE_VERIFIED. Absolute acceleration accuracy, absolute angular-rate accuracy, calibration-time motion detection, calibration motion rejection, long-duration thermal drift, final rover-frame alignment, shared-I2C concurrency, complete multisensor firmware operation, complete rover operation, motor-vibration behavior, encoder/IMU fusion, physical odometry accuracy, and ESP32/WiFi integration remain UNVERIFIED.
