...
Phase 3.2E adds an HC-SR04-only firmware target under `firmware/openrf1/hcsr04_bringup/` with Keil project `firmware/openrf1/keil/OpenRF1_HCSR04_Bringup.uvprojx`. Its output is isolated under `Objects_HCSR04_Bringup/` as `OpenRF1_HCSR04_Bringup.hex`. Validation-readiness tooling adds a strict shared-envelope parser, the mockable `capture-openrf1-hcsr04` command, a 512-byte JSONL line contract backed by a 513-byte firmware buffer, deterministic timeout/recovery fixtures, sanitized statistics, and a manual-evidence candidate lifecycle.
