...
Phase 3.2D isolates MPU6050 software bring-up from the Phase 3.2A BH1750 target, Phase 3.2B full-hardware target, and Phase 3.2C BMP280 target. It adds `OpenRF1_MPU6050_Bringup.uvprojx`, source under `firmware/openrf1/mpu6050_bringup/`, host-side tests for WHO_AM_I `0x68`, register configuration values, 14-byte burst decoding, raw-to-g/dps/temperature conversion, telemetry formatting, and target isolation. The planned manual wiring is GY-521/MPU6050 VCC -> OpenRF1 5 V, GND -> GND, SCL -> PB1/B1, SDA -> PC3/C3, and AD0 -> GND for address `0x68`; INT, XDA, XCL, and FSYNC remain disconnected. Repository automation does not flash hardware or access real GPIO/I2C/COM ports. MPU6050 ACK, WHO_AM_I, configuration readback, live IMU telemetry, calibration, axis orientation, shared-I2C concurrency, and complete full-hardware operation remain UNVERIFIED.
...
Phase 3.2F isolates ground-sensor software bring-up from the earlier OpenRF1 targets. It adds `OpenRF1_GroundSensors_Bringup.uvprojx`, source under `firmware/openrf1/ground_sensors_bringup/`, host-side tests for signal 1 / X1 / PC4, signal 2 / X2 / PC5, signal 3 / X3 / PB0, X4 conflict handling, floating input configuration, 5 ms sampling, 4-sample debounce, 50 ms JSONL telemetry, startup initialization from observed raw levels, numeric raw/debounced levels, target isolation, and no UTF-8 BOM in the Keil project. OpenRF1 vendor material locks the six-pin tracking connector order and vendor input mode as AUTHORITATIVE_VENDOR_DOCUMENTED. The design powers both TCRT5000 modules from 3.3 V, powers the Hall module from 5 V, and requires Hall S -> 10 kOhm / 15 kOhm divider -> protected PB0; direct Hall S -> PB0 is prohibited. Repository automation does not flash hardware or access real GPIO/timer/COM ports. Physical connector orientation, rail voltages, TCRT output topology, TCRT active polarity, Hall output voltage, Hall active polarity, surface response, magnetic response, real debounce suitability, actual 50 ms serial periodicity, and full-hardware operation remain UNVERIFIED.
